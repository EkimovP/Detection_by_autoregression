...
public:
	...
	void DrawGrid(vector<double> y_first_graph, int typefunc, vector<double> x_first_graph, int N, double y2, double T)
	{
		if (!init) return;
	...
	void DrawPlot(vector<double>& y_first_graph, int typefunc, vector<double>& x_first_graph, int N, double y2, double T, double t)
	{
		if (!init) return;
		...
		CPen data_pen1(PS_SOLID, 2, RGB(0, 0, 255));
